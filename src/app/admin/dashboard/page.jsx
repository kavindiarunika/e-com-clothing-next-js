  ...
  return (
    <main className="admin-page">
      <div className="page-header dashboard-heading">
        <div>
          <h1>Dashboard</h1>
          <p className="muted">A clear view of your store performance.</p>
        </div>
      ...
      <div className="stats-grid">
        <StatCard
          title="Total Revenue"
          value={`Rs. ${Number(data.stats.revenue).toLocaleString()}`}
          icon={Banknote}
        />

        <StatCard title="Total Orders" value={data.stats.orders} icon={ShoppingBag} />
        ...
        <StatCard title="Low Stock" value={data.stats.lowStock} icon={AlertTriangle} />
      </div>
          ...
          {products.length === 0 ? (
            ...
            products.map((product) => (
              <div className="recent-product" key={product.product_id}>
                ...
                <b>Rs. {Number(product.price || 0).toLocaleString()}</b>
              </div>
            ))
          )}
        </div>
